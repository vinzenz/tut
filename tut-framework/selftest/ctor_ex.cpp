...
//
// If the test ctor throws an exception, we shall terminate current group execution: there is no reason to
// continue to run tests in that group as they all going to fail and even if not their credibility is low anyway.
//
...
namespace tut
{
  ...
  {   
    ...
    { 
      dummy()
      {
        if( cnt++ == 1 ) throw std::runtime_error("dummy has throwed an exception");
      }
    };
    ...
    ctor_ex2() : factory("internal 2",tr)
    {
    }
  };
